...
if args.top_view:
    ...
    print('render mode set to top down')
else:
    render_mode = 'human'
...
# Path to preloaded key sequence inside container (mount your host file here)
KEYSEQ_FILE = "keyseq.json"  

# Load key sequence: list of {"t": seconds_from_start, "action":[left,right]}
def load_keyseq(path):
    if not os.path.exists(path):
        return []
    try:
        with open(path, "r") as f:
            seq = json.load(f)
        # sort by time, ensure action is a 2-list of floats
        seq = sorted([{"t": float(x["t"]), "action":[float(x["action"][0]), float(x["action"][1])]} for x in seq], key=lambda e: e["t"])
        return seq
    except Exception as e:
        print("Failed to load keyseq:", e)
        return []

KEYSEQ = load_keyseq(KEYSEQ_FILE)
USE_KEYSEQ = len(KEYSEQ) > 0
if USE_KEYSEQ:
    print("Using key sequence with", len(KEYSEQ), "entries from", KEYSEQ_FILE)
else:
    print("No key sequence found; using live keyboard input")

# internal pointer/state
_keyseq_idx = 0
_keyseq_start_time = None

# helper: get action from sequence for current elapsed time
def _action_from_keyseq(now):
    global _keyseq_idx, _keyseq_start_time
    if _keyseq_start_time is None:
        _keyseq_start_time = now
    elapsed = now - _keyseq_start_time
    # advance index while next events are in the past
    while _keyseq_idx + 1 < len(KEYSEQ) and KEYSEQ[_keyseq_idx + 1]["t"] <= elapsed:
        _keyseq_idx += 1
    if len(KEYSEQ) == 0:
        return np.array([0.0, 0.0])
    # current action is latest event whose time <= elapsed
    return np.array(KEYSEQ[_keyseq_idx]["action"], dtype=np.float32)
...
def update(dt):
    """
    Runs every frame. If a key-sequence JSON is provided and non-empty,
    actions are taken from the preloaded sequence based on elapsed time.
    Otherwise, falls back to live keyboard input from key_handler.
    """
    # determine action
    if USE_KEYSEQ:
        global frame_id
        # use wall-clock time so sequence works even if frame rate varies
        now = time.time()
        action = _action_from_keyseq(now)
        obs, reward, done, info = env.step(action)
        print('step_count = %s, reward=%.3f' % (env.unwrapped.step_count, reward))
        cv2.imwrite(f'./human_baseline/observation_manual_control_{frame_id:02d}.png', cv2.cvtColor(obs, cv2.COLOR_BGR2RGB))
        frame_id+=1
    else:
        print("no keyseq file found.")
#    obs, reward, done, info = env.step(action)
    if args.show_observations:
        # obs = cv2.resize(obs, (300, 300))
        #cv2.imshow("Observation", cv2.cvtColor(obs, cv2.COLOR_BGR2RGB))
        cv2.imwrite("observation_manual_control.png", cv2.cvtColor(obs, cv2.COLOR_BGR2RGB))
#        cv2.waitKey(1)
#        print('step_count = %s, reward=%.3f' % (env.unwrapped.step_count, reward))

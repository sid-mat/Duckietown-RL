...
if not (args.analyse_trajectories or args.visualize_salient_obj or args.reward_plots or args.visualize_dot_trajectories):
    ...
    out = cv2.VideoWriter('PPO_Demo_Video.avi', cv2.VideoWriter_fourcc(*'MJPG'), 30, (300,300))
    for i in range(5):
        obs = env.reset()
        ...
        while not done:
            ...
            # cv2.waitKey(1)
            frame = cv2.resize(obs[...,-3:].astype('float32'),(300,300))
...
print("entered the saliend map visualization")
if args.visualize_salient_obj:
    HIGH_RES_OUTPUT = True
    if HIGH_RES_OUTPUT:
        out = cv2.VideoWriter('salient_obj_video.avi', cv2.VideoWriter_fourcc(*"MJPG"), 30, (320, 320))
    else:
...
if args.reward_plots:
    ...
    for i in range(1):
        ...
        plt.grid('on')
        #plt.show()
        plt.savefig("reward_components.png", dpi=200)
        ...
        if len(npc_robot_pos) > 0:
            ...
            plt.xlabel("Time [s]")
            #plt.show()
            plt.savefig("robot_distance.png", dpi=200)
        ...
        plt.xlabel("Time [s]")
        # plt.show()
        plt.savefig("robot_speed.png", dpi=200)

...
class ForwardObstacleSpawnnigWrapper(ObstacleSpawningWrapper):
    def __init__(self, env, env_config):
        # Prevent parent from auto-spawning before child attributes are initialized.
        orig_spawn = env_config.get('spawn_obstacles', False)
        # Pass a copy of env_config with spawn_obstacles disabled
        temp_conf = dict(env_config)
        temp_conf['spawn_obstacles'] = False

        super(ForwardObstacleSpawnnigWrapper, self).__init__(env, temp_conf)
        self.env_config = env_config
        self.lateral_pos_perturb_half_width = env_config.get(
            'lateral_pos_perturb_half_width', 0.2 * self.simulator.road_tile_size)
        self.orientation_perturb_half_width = env_config.get(
            'orientation_perturb_half_width', np.pi / 4)
        #self.lateral_pos_perturb_half_width = 0.2 * self.simulator.road_tile_size
        #self.orientation_perturb_half_width = np.pi/4
        # Now if spawn_forward_obstacle is requested, spawn safely
        if self.env_config.get('spawn_forward_obstacle', False) or self.env_config.get('spawn_obstacles', False):
            self.safe_spawn_objects()
